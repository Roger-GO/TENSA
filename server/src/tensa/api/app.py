...
from tensa.api.routes.reports import router as reports_router
from tensa.api.routes.se import router as se_router
from tensa.api.routes.sessions import router as sessions_router
...
def _find_spa_dir() -> Path | None:
    ...
    """
    # ``app.py`` is at ``server/src/tensa/api/app.py``. Going up:
    #   parent      → ``server/src/tensa/api/``
    #   parent.parent → ``server/src/tensa/`` (the package root)
    ...
    #   api → tensa → src → server → <project root> → web/dist
    dev_dist = (
...
def make_app(
    *,
    ...
    if spa_dir is not None and (spa_dir / "index.html").is_file():
        ...
        log.warning(
            "No SPA bundle found at tensa/static or ../web/dist; "
