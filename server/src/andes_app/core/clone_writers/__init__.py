...
- :func:`andes_app.core.clone_writers.xlsx_writer.apply_edit` — ANDES-native
  ``.xlsx`` (one sheet per model, one row per device, columns are param names).
- :func:`andes_app.core.clone_writers.dyr_writer.apply_edit` — PSS/E ``.dyr``
  dynamics. Edits the target token *in place* preserving every sibling record
  (see the spike's CRITICAL writer requirement).
- :func:`andes_app.core.clone_writers.raw_writer.apply_edit` — PSS/E ``.raw``
...
resolved by the :class:`~andes_app.core.clone_manager.CloneManager` from the
...
def load_clone_write_index() -> dict[str, Any]:
    ...
    try:
        text = (
            resources.files("andes_app.core.clone_writers")
...
def apply_edit(
    ...
    *,
    ...
    from andes_app.core.clone_writers import dyr_writer, raw_writer, xlsx_writer
